package goorm.global.infra.filter;
...
@Slf4j
public class GoormBusJWTFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request, @NonNull HttpServletResponse response, @NonNull FilterChain filterChain) throws ServletException, IOException {
        ...
        if (requestURI.contains("/api/oauth2/login") || requestURI.contains("/api/oauth2/callback")) {
            ...
            if (jwtUtil.jwtVerify(accessToken, "access")) {
                throw new GoormBusException(ErrorCode.DUPLICATE_LOGIN_NOT_EXIST);
            }
        ...
        if (!jwtUtil.jwtVerify(accessToken, "access")) {
            throw new GoormBusException(ErrorCode.INVALID_ACCESS_TOKEN);
        }
